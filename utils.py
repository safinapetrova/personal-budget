def is_time_in_range(time_string, start_time, end_time):
    """Проверяет, находится ли время в заданном интервале [start_time, end_time]."""
    try:
...
def days_in_month(year, month):
    """Возвращает количество дней в месяце. Учитывает високосный год."""
...
def subtract_days_from_date(date_str, number):
    ...
    # Последовательно вычитаем дни, перехода через месяцы и годы при необходимости
...
def get_valid_latest_date(transactions):
    ...
    for transaction in transactions:
        date_val = transaction.get('date') # Получаем значение поля 'date', если есть
        if isinstance(date_val, str) and validate_and_parse_date(date_val) is not None:
            # Если значение - строка и её удаётся распарсить как дату, добавляем в список 
            valid_dates.append(date_val)
    if not valid_dates:
        return None # Нет валидных дат - возвращаем None
    return max(valid_dates) # Возвращаем самую позднюю дату (строковое сравнение работает для ГГГГ-ММ-ДД)
